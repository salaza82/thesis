...
Additionally, with the addition of a 2D binning scheme, $f_\mathrm{hit}$ and NN, the time needed to compute expected to grow.
...
Background considerations and source selection was identical to \Cref{sec:gd_databgd}, and no additional arguments are provided here.
...
\dfactor
Software was written to accommodate DM decay from dSphs, however decay profiles were not received from \LS by the time of writing this thesis.
...
Human time is more challenging to estimate as job allocation is stochastic and highly dependent on what other users are submitting, yet it is unlikely that all jobs would run simultaneously.
...
Asynchronous meaning that the instructions and computing within a function are independent and permitted to be out of sync with sibling computations.
...
Reducing the total instances of loading stages and distributing access to the reduced loads across multiple asynchronous threads is expected to reduce serial processing time and the overhead implicit to each job in addition to saving human time.
...
Within a job, \texttt{SOURCE} is kept fixed .
...
We have observed no significant deviation from the null, no DM, hypothesis, and so present our results in terms of upper limits on the annihilation cross-section for seventeen potential DM annihilation channels across four decades of DM mass.
