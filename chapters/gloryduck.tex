...
\gdLHratio
$\bm{\mathcal{D}_{\text{dSphs}}}$ is the totality of observations across experiments and dSphs.
...
Finally, $\hat{\hat{\bm{\nu}}}$ is the set of \J factors that maximize $\mathcal{L}$ for a fixed value of \sv.
...
% Results are computed for seven annihilation channels, $W^+W^-$, $ZZ$, $b\bar{b}$, $t\bar{t}$, $e^+e^-$, $\mu^+\mu^-$, and $\tau^+\tau^-$ over 62 $m_{\chi}$ values between 5 GeV and 100 TeV provided in~\cite{Cirelli}.
% The $\sv$ range is defined between $10^{-28}$ and $10^{-18}\rm{cm}^{3}\cdot \rm{s^{-1}}$, with 1001 logarithmically spaced values.
